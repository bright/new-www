...
published: true
---
Some time ago(15.10-16.10) I took part in the Sekurak Mega Hacking Party CTF contest. I have already created the first post about challanges that I had managed to solve, this post will desribe those which were pretty close ;)
...
Due to disappearing decided to use script interaction with a console which will allow for entering more commands:
...
So decided that the flag must be somewhere near the metadata of the table. Tried the DB version, current database and current user... Nothing interesting there... 

Also as a user CTF, I was not able to get info about information_schema which would contain more info. Suspected that...
...
It is closer :) Got the column name. Selecting it allows selecting a value from the flag table which was `Close`. 

<div class="image"><img src="/images/screenshot-2022-10-17-at-17.11.59.png" alt="Console 7" title="Console 7"  /> </div>

You might remember that the second value was `but not there yet` but it was not shown in the schema above. Tried several more approaches and here I have stopped. Turns out my thinking was correct but using the wrong query to do so. After deserializing the table to XML: 
...
"Decrypt" following text to find text origally written in Polish to find a flag: 
...
As "decrypt" word was there and the format was known to me, looks like it is not real encryption but base-64 encode ;););)
...
Hmmm tried to google translate it from Korean, the Chinese something about Confucius was there. Ok, wrong way. 

Tried to change the encoding to  binary representation, hex one etc... Tried to change to image... Have not seen the pattern that was there so abandoned that tasks. 

Turnes out that one had to convert to UTF-16 and observe that all chars are moved by 8 bits. After changing 8 at the start  to 0 message with CTF flag started to emerge.
...
So all in all Sekurak CTF was really interesing and well prepared. This was my first start so hope to find more flags in the future 🕵️
