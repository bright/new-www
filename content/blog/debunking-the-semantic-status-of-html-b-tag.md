...
As noted earlier, semantic tags are generally intended to improve accessibility (A11y), but the practical difference between `<b>` and `<strong>` tags in this area is less significant than one might expect. While these tags have distinct semantic meanings—support for their unique purposes is not consistently implemented across all screen readers. As a result, in many cases, they behave exactly the same \[6] (like a common text tag).
