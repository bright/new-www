...
The project was integrated with [Logstash](https://www.elastic.co/logstash) using a custom solution that sent log messages for analysis in [Kibana](https://www.elastic.co/kibana). Unlike other remote logging tools (e.g. [DataDog](https://www.datadoghq.com/) or [Crashlytics](https://firebase.google.com/products/crashlytics)) that can use a mapping file generated by R8/ProGuard to de-obfuscate logs (those tools come Gradle plugins for this purpose), the Logstash integration lacked this mechanism. As a result, the class names used in SLF4J loggers were obfuscated in the log messages visible in Kibana, making it challenging to understand where the logs originated.
