...
Every mobile developer must have at least heard about Flutter lately. It's a new Google's framework for creating cross-platform
applications. So far, I haven't tried to create any cross-platform projects so I wanted to give it a try. This won't be a next
...
Before I started implementing my first application, I had thought it would be wise to prepare myself for Dart syntax.
...
only UI elements, which are compiled, but the whole thing is. Flutter doesn't use a special bridge to communicate between
...
Without writing even a single application using Flutter it's pretty easy to understand what is going on here:
...
As you can see it is pretty simple to understand what is going on here. At first, combining UI and logic in a single place seems strange,
...
When you add image, you need to specify it in `pubspec.yaml` file and I've made a typo there. Fortunately, page's like:
