...
Let’s keep an eye on the mentioned Contracts module. Using smart contracts places you in a kind of a sandbox. If you write your own runtime module you need to be very careful and strictly follow the rule “check first, write last”. At first you need to make sure that any prerequisites are met and making a change will not cause any errors. Only then can you do the changes. Smart contracts on the other hand are revertable by design.

You are also charged for the smart contracts. There is a gas fee for the computational resources and a storage rent for the blockchain’s storage used.
...
On the other hand, in the real world use case you would need to get your hands dirty and write your own sart contract or a runtime module. In addition to the front end app you would also need some backend with a regular database to store and manage all the details which blockchain is not interested in. Polkadot JS API can also be really helpful with it.
