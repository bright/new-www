...
In the [recent post](https://brightinventions.pl/blog/modern-way-getting-started-react-typescript/) we discussed how we can start the new React project with TypeScript to benefit from all the goodies of type checks and eliminate the whole class of errors and typos from our codebase. Now let's see how we can actually apply TypeScript-based type definitions into our React project to get most of it.
...
As well as we're not allowed to monkey-patch it, that is to add the new properties there:
...
Until there is a [proper built-in support for deep Readonly](https://github.com/Microsoft/TypeScript/issues/13923), the best we can (and should) do is to ensure on our own that all the properties of both props and state of our components are also marked as `Readonly<>` (as well as its inner properties, ad infinitum):
...
![State correctness is enforced](/images/react-ts/props-frozen.png)￼￼
